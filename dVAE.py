...
class dVAE:
    ...
    def __init__(self, continuous, hu_encoder, hu_decoder, n_latent, x_train, b1=0.95, b2=0.999, batch_size=100, learning_rate=0.001, lam=0):
        ...
        W_hh = theano.shared(create_weight(hu_encoder, n_latent), name='W_hh')
        b_hh = theano.shared(create_bias(n_latent), name='b_hh')

        W_hmu = theano.shared(create_weight(n_latent, n_latent), name='W_hmu')
        b_hmu = theano.shared(create_bias(n_latent), name='b_hmu')

        W_hsigma = theano.shared(create_weight(n_latent, n_latent), name='W_hsigma')
        ...
        self.params = OrderedDict([("W_xh", W_xh), ("b_xh", b_xh), ("W_hh", W_hh), ("b_hh", b_hh),
    ...
    def encoder(self, x):
        ...
        #h_power = T.pow(h_latent, 2)
        #h_peaked = h_power / T.sum(h_latent)

        mu, log_sigma = self.encode_from_last_layer(h_clip)
